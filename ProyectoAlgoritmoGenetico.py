...
def parserAristas(file):
    ...
    for i, linea in enumerate (lineas):
        nums = linea.split(", ")
        aristas.append(arista(i, str(nums[0]), str(nums[1]), str(nums[2]), float(nums[3]), float(nums[4]), float(nums[5]), float(nums[6]), float(nums[7]), float(nums[8])))
    return aristas
class arista:
...
class cromosoma:
    ...
    def aptitud( self ):
        apt = 0
        orig = sorted(self.getaristas(), key=lambda x: x.getid(), reverse=False)
        for a in orig:
            apt += self.costo(a)
        return apt
    ...
    def index( self, nodo ):
        return self.__aristas.index(nodo)
...
padres = []
def buscarM(origen, aristas):
    for arista in aristas:
        if arista.getdest() == origen:
            padres.append(arista)
            buscarM(arista.getorg(), aristas)
    return padres
def main():
    aristas = parserAristas("aristas.txt")
    ...
    for arista in sorted(respuestaFinal.getaristas(), key=lambda x: x.getid(), reverse=True):
        dot2.node(arista.getorg())
        dot2.edge(arista.getorg(), arista.getdest(), str(arista.gettrf()))
    lista = sorted(respuestaFinal.getaristas(), key=lambda x: x.getid(), reverse=True)
    padre = None
    while padre is None:
        padre = buscarM(lista[0].getorg(), aristas)
    if padre is not None:
        if len(padre) > 0:
            for hijo in padre:
                if hijo not in respuestaFinal.getaristas():
